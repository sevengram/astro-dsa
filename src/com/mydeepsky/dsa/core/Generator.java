...
public class Generator {
    ...
    public List<CelestialObject> result = new ArrayList<CelestialObject>();
    ...
    public void generateList() {
        List<CelestialObject> early = new ArrayList<CelestialObject>();
        List<CelestialObject> mid = new ArrayList<CelestialObject>();
        List<CelestialObject> late = new ArrayList<CelestialObject>();
    ...
    public String saveList() throws IOException {
        String date = String.format("%02d", geoInfo.getYear())
                + String.format("%02d", geoInfo.getMonth())
                + String.format("%02d", geoInfo.getDate());

        String skySafariFile = date + ".skylist";

        printSkySafariList(result, skySafariFile);
        return skySafariFile;
    }
    ...
    private void printSkySafariList(Collection<CelestialObject> objects, String filename)
        ...
        File path = new File(DirManager.SD_PATH + "/SkySafari Pro/Observing Lists");
        if (!path.exists()) {
            path.mkdirs();
        }

        File out_file = new File(DirManager.SD_PATH + "/SkySafari Pro/Observing Lists/" + filename);
        BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(
                new FileOutputStream(out_file)));
